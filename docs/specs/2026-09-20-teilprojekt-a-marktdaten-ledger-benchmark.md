...
| `poller.py` | **256** | 225 *(Gesamt-Fixrunde A2)* | Einziger Ort, an dem der Live-Thread läuft: Kerzen holen/speichern, `resolve_pending()`, Equity-Schnappschuss, `prune_candles()` (A-16, V-7), Backoff, Thread-Verwaltung (`run_forever`/`start`). Ein erheblicher Teil sind Docstrings, die das *Warum* tragen (`except Exception` statt `except BinanceError`; Iteration über `pc.ctx.specs` statt `cfg.market_symbols`; V-4: kein früher `return` mehr bei fehlgeschlagener Zeitabfrage; V-6: nur wirklich neue Kerzen). Mit den sieben Behebungen der Gesamt-Fixrunde lag das Modul bei **349** Zeilen — über der harten Marke. Geteilt entlang der Naht, die der Modul-Docstring schon zog: den Zyklus *fahren* bleibt hier, den Betriebszustand *feststellen und festschreiben* ging nach `pollstate.py`. |
| `dashboard.py` | **217** | 191 *(neu in der Tabelle, Gesamt-Fixrunde A2)* | Rechnet, was das Dashboard über sich selbst aussagt (`build_live_ledger`, `last_prices`, `build_status`, `build_health`). Gewachsen um `lese_schnappschuss()` (V-2: Kasse und Positionen aus **einer** Transaktion — ohne Klammer entstand ein Scheinverlust von 812,88 USDC, der über `daily_loss_pct()` einen echten Kill Switch auslösen kann) und `market_data_ansicht()` (K-2: `market_data.{status,age_s}` in `/api/status`, `age_s` als JSON-Zahl, `inf` → `null`). |
...
> ohne eigene Tabellenzeile:** `pollstate.py` (**118** Zeilen, 100 nicht leer) trägt den
...
> `tageswechsel`); `marketdata.py` wuchs auf **181** Zeilen (144 nicht leer) um
