...
Neu gemessen am 2026-09-22, Gesamt-Fixrunde zu A2 (`wc -l app/aitra/*.py`, nicht-leer über
`grep -c '[^[:space:]]'`) — **zehn von achtzehn** Modulen liegen über dem Richtwert, keines über
der harten Marke. Zwei Zahlen der Vorgängermessung stimmten nicht mehr (`ledger.py` stand mit
236/200 in der Tabelle, gemessen sind 272/233; `web.py` mit 245/210, gemessen 250/215) — der
Wächter sichert nur die *Existenz* einer Zeile zu, nicht die Zahl, deshalb fiel es nicht auf. `test_modulgroesse.py` sichert seit Fixrunde 1 zusätzlich zu, dass jedes Modul
...
| `ledger.py` | **272** | 233 | Gewachsen um die strenge `mark()` (wirft bei Position ohne Marktpreis) und `last_marks`, in A2/Aufgabe 5 um `restore(positions, cash)` (A-14: Kasse/Positionen aus dem Journal setzen statt über `apply()` zu erarbeiten, für den Start eines neuen Prozesses). Fast nur Docstring, der das *Warum* trägt. |
| `web.py` | **250** | 215 *(A2, Aufgabe 6, Fixrunde 1)* | HTTP-Huelle: App-Aufbau, Sicherheitskopfzeilen, Anmeldung, Kill Switch, `decisions`/`events`/`version`, sowie `POST /api/risk/check` (bucht ueber `execute_proposal()` - bleibt hier, A-6b praeft die aufrufende Datei). `status()`/`health()` sind seit der Teilung mit `dashboard.py` (Ruling des Koordinators zur 300-Zeilen-Rueckfrage) nur noch duenne Huellen, die `jsonify()`en, was dort berechnet wird. Fixrunde 1 (Koordinator/Reviewer, Punkt 3): zwei eigene Start-Verbindungen (Init-Block, `specs`-Aufbau) schliessen jetzt ausdruecklich per `try`/`finally` - `with db.connect(...) as conn:` beendet bei `sqlite3.Connection` nur die Transaktion, nicht die Verbindung selbst. |
| `execute.py` | **253** | 222 | Gewachsen um `reject_decision()` an sieben Stellen, `pending_ref_price` und `resolve_pending()`/`expire_stale_pending()` (E-010, Weg A: schwebende Vorschläge ohne Neubemessung aus dem Live-Poller auflösen, A2/Aufgabe 5). *Gesamt-Fixrunde A2, B-2:* die Grenze `candle.open_time <= pending_since_ms` in `resolve_pending()` samt Begründung — E-006 war im Livepfad verletzt, gefüllt wurde zum *open* der Entscheidungskerze selbst. |
...
| `config.py` | **246** | 203 *(neu in der Tabelle, A2, Aufgabe 5)* | Elf neue Felder für den Live-Betrieb (Spec 18: `market_data_enabled`, `market_interval`, `market_poll_s`, die vier Veraltet-/Uhrversatz-Schwellen, `fee_bps`, `slippage_bps`, `benchmark_symbol`, `candle_retention_days`), jedes mit eigenem, begründetem Leser (`_bool`, `_interval`, `_int_range`, `_benchmark_symbol`) statt einer generischen Parse-Funktion, weil jedes Feld eigene Grenzen und Fehlermeldungen braucht. |
| `poller.py` | **270** | 239 *(Gesamt-Fixrunde A2 + Nachreview)* | Einziger Ort, an dem der Live-Thread läuft: Kerzen holen/speichern, `resolve_pending()`, Equity-Schnappschuss, `prune_candles()` (A-16, V-7), Backoff, Thread-Verwaltung (`run_forever`/`start`). *Nachreview, N-2:* der Kommentar, warum die zwei Zeilen `last_server_time_ms = server_time()` / `last_time_check_ms = now_ms()` in **dieser** Reihenfolge stehen müssen — vertauscht läuft die Projektion der Serverzeit voraus und eine unfertige Kerze kann als geschlossen durchgehen. Ein erheblicher Teil sind Docstrings, die das *Warum* tragen (`except Exception` statt `except BinanceError`; Iteration über `pc.ctx.specs` statt `cfg.market_symbols`; V-4: kein früher `return` mehr bei fehlgeschlagener Zeitabfrage; V-6: nur wirklich neue Kerzen). Mit den sieben Behebungen der Gesamt-Fixrunde lag das Modul bei **349** Zeilen — über der harten Marke. Geteilt entlang der Naht, die der Modul-Docstring schon zog: den Zyklus *fahren* bleibt hier, den Betriebszustand *feststellen und festschreiben* ging nach `pollstate.py`. |
| `dashboard.py` | **226** | 200 *(neu in der Tabelle, Gesamt-Fixrunde A2)* | Rechnet, was das Dashboard über sich selbst aussagt (`build_live_ledger`, `last_prices`, `build_status`, `build_health`). Gewachsen um `lese_schnappschuss()` (V-2: Kasse und Positionen aus **einer** Transaktion — ohne Klammer entstand ein Scheinverlust von 812,88 USDC, der über `daily_loss_pct()` einen echten Kill Switch auslösen kann) und `market_data_ansicht()` (K-2: `market_data.{status,age_s}` in `/api/status`, `age_s` als JSON-Zahl, `inf` → `null`). *Nachreview:* Vermerk am Max-Drawdown, wie ein Aggregat **ohne** `float` ginge (Höchststand als kanonischer TEXT in `state` fortschreiben) — als Notiz, nicht gebaut. |
...
> **Zwei neue Module aus der Gesamt-Fixrunde A2, beide unter dem 200er-Richtwert und deshalb
> ohne eigene Tabellenzeile:** `pollstate.py` (**124** Zeilen, 105 nicht leer) trägt den
> veränderlichen Zustand eines Live-Laufs (`PollerContext`, `PollOutcome`) und die drei
> Funktionen, die ihn feststellen und festschreiben (`check_staleness`, `apply_staleness`,
> `tageswechsel`) — seit dem Nachreview mit eigenem Trennungswächter (`test_poller_und_pollstate_sind_wirklich_getrennt`, symmetrisch über `ast`-Namensmengen plus Identitätsprüfung der re-exportierten Namen, N-1); `marketdata.py` wuchs auf **181** Zeilen (144 nicht leer) um
> `projizierte_serverzeit()` (B-1). Die Teilung von `poller.py` war nicht geplant, sondern
> durch die harte 300er-Marke erzwungen — sie steht unter Vorbehalt der Rücksprache.
...
> `web.py` liegt danach bei **226 Zeilen** (nach der Fixrunde 1 dieser Aufgabe, siehe
> Verbindungslecks unten: **245 Zeilen**), `test_modulgroesse.py` deckt auch diese Trennung
> zusätzlich über einen eigenen Test ab (`test_web_und_dashboard_sind_wirklich_getrennt`).
